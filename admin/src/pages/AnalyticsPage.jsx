...
export default function AnalyticsPage() {
    ...
    const sevColors = { critical: '#ff3b3b', high: '#ff6b35', medium: '#ffaa00', low: '#00cc66', unknown: '#555' };
                            ...
                            {donutSlices.map(s => (
                                <div key={s.level} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                    ...
                                    <span style={{ fontSize: 12, color: 'var(--color-text-secondary)', fontWeight: 500 }}>
                                        {s.level.charAt(0).toUpperCase() + s.level.slice(1)}: {s.count} ({(s.pct * 100).toFixed(0)}%)
                                    </span>
                                </div>
                            ))}
                        </div>
